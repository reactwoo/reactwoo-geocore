...
= 1.8.158 =
* Cloud entitlement is now grant-OR: a commercially valid Decision Cloud snapshot or a standalone licence can entitle covered capabilities. Connecting Cloud no longer replaces standalone access by itself.
...
= 1.8.152 =
* **Cloud WP16:** Detect local visibility rules, slots, variants, experiments and commerce rules; show an import preview without contacting Cloud. Explicit **Import to ReactWoo Cloud** writes a local backup and POSTs supported resources. **Switch to Cloud-managed** is a second action. Pairing never flips `management_mode`. Disconnect keeps WordPress content, cached manifests and the migration backup.

= 1.8.151 =
* **Cloud WP15:** `RWGC_Entitlements` facade (`allows()` / `limit()`) maps standalone licenses or cached Cloud entitlements. Feature code must not inspect Stripe IDs. Heartbeat stores the Cloud snapshot; disconnect clears it. Visitor rendering still never calls Cloud.
...
= 1.8.147 =
* **Elementor:** Seed `tabs_controls.content` before the inspector opens. Empty widgets-config left document widgets without tabs, so Elementor 4.2 crashed on `.content` and never requested the single-widget controls.
