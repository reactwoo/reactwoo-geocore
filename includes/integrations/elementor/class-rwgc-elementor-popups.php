...
function forceClosePopup(pid){pid=norm(pid);if(!pid){return;}markPopupUserClosed(pid);try{var doc=getPopupDoc(pid);if(doc&&typeof doc.getModal==='function'){doc.getModal().hide();return;}}catch(e){}try{var modals=document.querySelectorAll('.elementor-popup-modal');for(var i=0;i<modals.length;i++){var mid=popupIdFromModal(modals[i]);if(mid&&String(mid)===String(pid)){var m=modals[i];m.style.display='none';m.setAttribute('aria-hidden','true');try{document.body.classList.remove('elementor-popup-modal-open');}catch(e3){}}}}catch(e2){}}
...
function applyOpen(orig,scope,args){var pid=resolvePopupIdFromArgs(args);if(pid&&(shouldBlockOpen(pid)||isPopupUserClosed(pid))){return false;}return orig.apply(scope,args);}
function wrapClose(orig,scope,args){var pid=resolveClosePopupId(args);if(pid){markPopupUserClosed(pid);}return orig.apply(scope,args);}
...
function forceShowModalDom(pid){if(isPopupUserClosed(pid)){return false;}var modal=findPopupModal(pid);if(!modal){return false;}modal.style.setProperty('display','flex','important');modal.style.setProperty('visibility','visible','important');modal.style.setProperty('opacity','1','important');modal.removeAttribute('aria-hidden');try{document.body.classList.add('elementor-popup-modal-open');}catch(e){}return isPopupVisible(pid);}
function openAllowedPopup(pid){pid=norm(pid);if(!pid||shouldBlockOpen(pid)){return false;}if(isPopupUserClosed(pid)){dbg('skip open user-closed',pid);return false;}clearEgpReopenBlock(pid);clearPopupDisableStorage(pid);patchRuntime();var doc=ensurePopupDocument(pid);var mod=window.elementorProFrontend&&elementorProFrontend.modules&&elementorProFrontend.modules.popup;dbg('open attempt',pid,'doc=',!!doc,'ready=',isPopupDocReady(doc),'egp_closed=',isPopupUserClosed(pid));if(doc&&isPopupDocReady(doc)&&typeof doc.showModal==='function'){try{doc.showModal();}catch(e){dbg('showModal error',pid,e&&e.message?e.message:e);}}else if(mod&&typeof mod.showPopup==='function'){mod.showPopup({id:pid});}if(!isPopupVisible(pid)&&doc){forceOpenViaModal(doc,pid);}if(!isPopupVisible(pid)){forceShowModalDom(pid);}return isPopupVisible(pid);}
