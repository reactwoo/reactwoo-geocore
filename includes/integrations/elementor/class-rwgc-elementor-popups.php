...
class RWGC_Elementor_Popups {
	...
	private static function build_popup_runtime_script( $map, $visitor, $blocked ) {
		...
		return "(function(){\n"
			...
			. "function disableElementorPopup(pid){pid=norm(pid);if(!pid){return;}markDismissed(pid);try{var doc=getPopupDoc(pid);if(doc&&typeof doc.disable==='function'){doc.disable();}}catch(e){}}\n"
			...
			. "function patchModalHide(doc,docId){if(!doc||typeof doc.getModal!=='function'){return;}try{var modal=doc.getModal();if(!modal||typeof modal.hide!=='function'||modal.__rwgcHidePatch){return;}var oh=modal.hide;modal.hide=function(){if(docId){disableElementorPopup(docId);}return oh.apply(modal,arguments);};modal.__rwgcHidePatch=1;}catch(e){}}\n"
			. "function patchSingleDocument(doc,docKey){if(!doc){return false;}var docId=resolveDocId(doc,docKey);var patched=false;if(typeof doc.showModal==='function'&&!doc.__rwgcShowModalPatch){var os=doc.showModal;doc.showModal=function(){if(docId&&!shouldShowForPopup(docId)){return;}var ret=os.apply(doc,arguments);patchModalHide(doc,docId);return ret;};doc.__rwgcShowModalPatch=1;patched=true;}else{patchModalHide(doc,docId);}return patched;}\n"
			. "function forceClosePopup(pid){pid=norm(pid);if(!pid){return;}disableElementorPopup(pid);try{var doc=getPopupDoc(pid);if(doc&&typeof doc.getModal==='function'){doc.getModal().hide();return;}}catch(e){}try{var modals=document.querySelectorAll('.elementor-popup-modal');for(var i=0;i<modals.length;i++){var mid=popupIdFromModal(modals[i]);if(mid&&String(mid)===String(pid)){var m=modals[i];m.style.display='none';m.setAttribute('aria-hidden','true');}}}catch(e2){}}\n"
			...
			. "function wrapClose(orig,scope,args){var pid=resolveClosePopupId(args);if(pid){disableElementorPopup(pid);}return orig.apply(scope,args);}\n"
			...
			. "function onPopupHide(popupId){var pid=norm(popupId);if(pid){disableElementorPopup(pid);}}\n"
			. "function bindPopupEvents(){if(window.__rwgcPopupGeoEventPatch){return;}window.__rwgcPopupGeoEventPatch=1;var jq=window.jQuery;if(!jq){return;}var onHide=function(evt,popupId){onPopupHide(popupId);};var onShow=function(evt,popupId,docInst){var pid=norm(popupId);if(docInst){patchSingleDocument(docInst,pid);}if(!pid){return;}var m=meta(pid);if(m&&m.rwgc_show===false){setTimeout(function(){forceClosePopup(pid);},0);}};var targets=[];if(window.elementorFrontend&&elementorFrontend.elements){if(elementorFrontend.elements.\$document){targets.push(elementorFrontend.elements.\$document);}if(elementorFrontend.elements.\$window){targets.push(elementorFrontend.elements.\$window);}}targets.push(jq(document));targets.forEach(function(\$el){if(\$el&&typeof \$el.on==='function'){\$el.on('elementor/popup/hide',onHide).on('elementor/popup/show',onShow);}});}\n"
