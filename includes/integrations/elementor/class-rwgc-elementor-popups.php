...
class RWGC_Elementor_Popups {
	...
	private static function build_popup_runtime_script( $map, $visitor, $blocked ) {
		...
		return "(function(){\n"
			...
			. "var dismissKey='rwgc_popup_dismissed';\n"
			...
			. "function readDismissed(){try{var raw=sessionStorage.getItem(dismissKey);return raw?JSON.parse(raw):{};}catch(e){return {};}}\n"
			. "function isDismissed(pid){if(!pid){return false;}var map=readDismissed();return !!map[String(pid)];}\n"
			. "function markDismissed(pid){if(!pid){return;}var map=readDismissed();map[String(pid)]=Date.now();try{sessionStorage.setItem(dismissKey,JSON.stringify(map));}catch(e){}}\n"
			...
			. "function shouldShowForPopup(pid){if(isDismissed(pid)){return false;}var m=meta(pid);if(!m){return true;}return popupShouldDisplay(m);}\n"
			...
			. "function patchModalHide(doc,docId){if(!doc||typeof doc.getModal!=='function'){return;}try{var modal=doc.getModal();if(!modal||typeof modal.hide!=='function'||modal.__rwgcHidePatch){return;}var oh=modal.hide;modal.hide=function(){if(docId){markDismissed(docId);}return oh.apply(modal,arguments);};modal.__rwgcHidePatch=1;}catch(e){}}\n"
			. "function forceClosePopup(pid){pid=norm(pid);if(!pid){return;}markDismissed(pid);try{var docs=getPopupDocuments();if(docs&&docs[pid]&&typeof docs[pid].getModal==='function'){docs[pid].getModal().hide();return;}}catch(e){}}\n"
			...
			. "function wrapClose(orig,scope,args){var pid=resolveClosePopupId(args);if(pid){markDismissed(pid);}return orig.apply(scope,args);}\n"
			. "function patchPopupDocuments(){var docs=getPopupDocuments();if(!docs){return false;}var patched=false;Object.keys(docs).forEach(function(docKey){var doc=docs[docKey];if(!doc){return;}var docId=resolveDocId(doc,docKey);if(typeof doc.showModal==='function'&&!doc.__rwgcShowModalPatch){var os=doc.showModal;doc.showModal=function(){if(docId&&!shouldShowForPopup(docId)){return;}var ret=os.apply(doc,arguments);patchModalHide(doc,docId);return ret;};doc.__rwgcShowModalPatch=1;patched=true;}else{patchModalHide(doc,docId);}});return patched;}\n"
			...
			. "function onPopupHide(popupId){var pid=norm(popupId);if(pid){markDismissed(pid);}}\n"
			. "function bindPopupEvents(){if(window.__rwgcPopupGeoEventPatch){return;}window.__rwgcPopupGeoEventPatch=1;var jq=window.jQuery;if(!jq){return;}var onHide=function(evt,popupId){onPopupHide(popupId);};var onShow=function(evt,popupId){var pid=norm(popupId);if(!pid||shouldShowForPopup(pid)){return;}setTimeout(function(){forceClosePopup(pid);},0);};var targets=[];if(window.elementorFrontend&&elementorFrontend.elements){if(elementorFrontend.elements.\$document){targets.push(elementorFrontend.elements.\$document);}if(elementorFrontend.elements.\$window){targets.push(elementorFrontend.elements.\$window);}}targets.push(jq(document));targets.forEach(function(\$el){if(\$el&&typeof \$el.on==='function'){\$el.on('elementor/popup/hide',onHide).on('elementor/popup/show',onShow);}});}\n"
			. "function installCloseCapture(){if(window.__rwgcPopupCloseCapture){return;}window.__rwgcPopupCloseCapture=1;document.addEventListener('click',function(e){var t=e.target;if(!t||!t.closest){return;}var modal=t.closest('.elementor-popup-modal');if(!modal){return;}var closeBtn=t.closest('.dialog-close-button');var overlay=t.classList&&t.classList.contains('dialog-widget-overlay');if(!closeBtn&&!overlay){return;}var id=modal.getAttribute('data-elementor-id');if(id){markDismissed(norm(id));}},true);}\n"
			. "if(!window.__rwgcPopupGeoHideListener){window.__rwgcPopupGeoHideListener=1;window.addEventListener('elementor/popup/hide',function(evt){if(evt&&evt.detail){onPopupHide(evt.detail.id);}},true);}\n"
			. "function patch(){var docsOk=patchPopupDocuments();var modOk=patchModule();bindPopupEvents();installCloseCapture();if(docsOk||modOk){window.__rwgcPopupGeoPatched=1;return true;}return !!window.__rwgcPopupGeoPatched;}\n"
