...
class RWGC_Elementor_Popups {
	...
	private static function build_popup_runtime_script( $map, $visitor, $blocked ) {
		...
		return "(function(){\n"
			...
			. "function isDismissed(pid){if(!pid||!meta(pid)){return false;}var map=readDismissed();return !!map[String(pid)];}\n"
			. "function markDismissed(pid){if(!pid||!meta(pid)){return;}var map=readDismissed();map[String(pid)]=Date.now();try{sessionStorage.setItem(dismissKey,JSON.stringify(map));}catch(e){}}\n"
			...
			. "function shouldShowForPopup(pid){var m=meta(pid);if(!m){return true;}if(isDismissed(pid)){return false;}return popupShouldDisplay(m);}\n"
			...
			. "function wrapClose(orig,scope,args){var pid=norm(args.length?args[0]:null);if(pid){markDismissed(pid);}return orig.apply(scope,args);}\n"
			...
			. "if(mod&&typeof mod.showPopup==='function'&&!mod.__rwgcPopupGeoPatch){var o=mod.showPopup;mod.showPopup=function(){return apply(o,this,arguments);};if(typeof mod.triggerPopup==='function'){var t=mod.triggerPopup;mod.triggerPopup=function(){return apply(t,this,arguments);};}if(typeof mod.closePopup==='function'){var c=mod.closePopup;mod.closePopup=function(){return wrapClose(c,this,arguments);};}mod.__rwgcPopupGeoPatch=1;window.__rwgcPopupGeoPatched=1;return true;}"
			. "if(typeof elementorFrontend!=='undefined'){var docRoot=elementorFrontend.documents&&elementorFrontend.documents.manager&&elementorFrontend.documents.manager.documents&&elementorFrontend.documents.manager.documents[0];if(docRoot&&typeof docRoot.showPopup==='function'&&!docRoot.__rwgcPopupGeoPatch){var ds=docRoot.showPopup;docRoot.showPopup=function(){return apply(ds,this,arguments);};if(typeof docRoot.triggerPopup==='function'){var dt=docRoot.triggerPopup;docRoot.triggerPopup=function(){return apply(dt,this,arguments);};}if(typeof docRoot.closePopup==='function'){var dc=docRoot.closePopup;docRoot.closePopup=function(){return wrapClose(dc,this,arguments);};}docRoot.__rwgcPopupGeoPatch=1;window.__rwgcPopupGeoPatched=1;return true;}}"
			. "return false;}\n"
			. "function onPopupHide(popupId){var pid=norm(popupId);if(pid){markDismissed(pid);}}\n"
			. "if(window.jQuery&&window.jQuery(document)&&!window.__rwgcPopupGeoEventPatch){window.__rwgcPopupGeoEventPatch=1;"
			. "window.jQuery(document).on('elementor/popup/hide',function(evt,popupId){onPopupHide(popupId);});"
			. "window.jQuery(document).on('elementor/popup/show',function(evt,popupId){var pid=norm(popupId);if(!pid||shouldShowForPopup(pid)){return;}setTimeout(function(){suppressPopup(pid);},0);});}\n"
