...
function isPopupUserClosed(pid){if(!pid){return false;}try{return sessionStorage.getItem('egp_closed_'+String(pid))==='1';}catch(e){return false;}}
function markPopupUserClosed(pid){if(!pid){return;}pid=String(norm(pid));suppressReopenBriefly(pid);fallbackDone[pid]=1;try{sessionStorage.setItem('egp_closed_'+pid,'1');}catch(e){}try{if(window.elementorFrontend&&elementorFrontend.storage){elementorFrontend.storage.set('popup_'+pid+'_disable',true);}}catch(e2){}}
...
function patchSingleDocument(doc,docKey){if(!doc){return false;}var docId=resolveDocId(doc,docKey);if(typeof doc.showModal!=='function'||doc.__rwgcShowModalPatch){return false;}var os=doc.showModal;doc.showModal=function(){if(docId&&shouldBlockOpen(docId)){return;}if(docId&&isPopupUserClosed(docId)){return;}if(docId&&isSuppressingReopen(docId)){return;}return os.apply(doc,arguments);};doc.__rwgcShowModalPatch=1;return true;}
function hideForcedModalDom(pid){pid=norm(pid);if(!pid){return;}var sel='#elementor-popup-modal-'+pid+',.elementor-popup-modal[data-elementor-id="'+pid+'"]';var nodes=document.querySelectorAll(sel);for(var i=0;i<nodes.length;i++){var m=nodes[i];m.style.setProperty('display','none','important');m.style.setProperty('visibility','hidden','important');m.style.setProperty('opacity','0','important');m.setAttribute('aria-hidden','true');}try{document.body.classList.remove('elementor-popup-modal-open');}catch(e){}}
function forceClosePopup(pid){pid=norm(pid);if(!pid){return;}markPopupUserClosed(pid);try{var doc=getPopupDoc(pid);if(doc&&typeof doc.getModal==='function'){doc.getModal().hide();}}catch(e){}try{var mod=window.elementorProFrontend&&elementorProFrontend.modules&&elementorProFrontend.modules.popup;if(mod&&typeof mod.closePopup==='function'&&!mod.__rwgcClosing){mod.__rwgcClosing=1;try{mod.closePopup({id:pid});}catch(eClose){}mod.__rwgcClosing=0;}}catch(e2){}hideForcedModalDom(pid);dbg('forceClose',pid,'visible=',isPopupVisible(pid));}
...
function applyOpen(orig,scope,args){var pid=resolvePopupIdFromArgs(args);if(pid&&(shouldBlockOpen(pid)||isPopupUserClosed(pid)||isSuppressingReopen(pid))){return false;}return orig.apply(scope,args);}
function wrapClose(orig,scope,args){if(scope&&scope.__rwgcClosing){return orig.apply(scope,args);}var pid=resolveClosePopupId(args);if(pid){markPopupUserClosed(pid);hideForcedModalDom(pid);}return orig.apply(scope,args);}
...
function patchFinalShowPopup(){var mod=window.elementorProFrontend&&elementorProFrontend.modules&&elementorProFrontend.modules.popup;if(!mod||typeof mod.showPopup!=='function'){return false;}var current=mod.showPopup;if(current.__rwgcFinalOuter===current){return true;}var inner=current;var wrapped=function(){var pid=resolvePopupIdFromArgs(arguments);if(pid&&shouldBlockOpen(pid)){dbg('block showPopup',pid);return false;}if(pid&&isPopupUserClosed(pid)){dbg('skip showPopup user-closed',pid);return false;}if(pid&&isSuppressingReopen(pid)){dbg('skip showPopup suppress',pid);return false;}if(pid&&!shouldBlockOpen(pid)&&!isPopupUserClosed(pid)){clearEgpReopenBlock(pid);clearPopupDisableStorage(pid);}return inner.apply(this,arguments);};wrapped.__rwgcFinalOuter=wrapped;wrapped.__rwgcInner=inner;mod.showPopup=wrapped;dbg('final showPopup wrap');return true;}
function bindPopupEvents(){if(window.__rwgcPopupGeoEventPatch){return;}window.__rwgcPopupGeoEventPatch=1;var jq=window.jQuery;if(!jq){return;}var onShow=function(evt,popupId,docInst){var pid=norm(popupId);if(docInst){patchSingleDocument(docInst,pid);}if(pid&&isPopupUserClosed(pid)){forceClosePopup(pid);}};var onHide=function(evt,popupId){var pid=norm(popupId);if(pid){markPopupUserClosed(pid);hideForcedModalDom(pid);}};var targets=[];if(window.elementorFrontend&&elementorFrontend.elements){if(elementorFrontend.elements.$document){targets.push(elementorFrontend.elements.$document);}if(elementorFrontend.elements.$window){targets.push(elementorFrontend.elements.$window);}}targets.push(jq(document));targets.forEach(function($el){if($el&&typeof $el.on==='function'){$el.on('elementor/popup/show',onShow);$el.on('elementor/popup/hide',onHide);}});}
function installCloseCapture(){if(window.__rwgcPopupCloseCapture){return;}window.__rwgcPopupCloseCapture=1;document.addEventListener('click',function(e){var t=e.target;if(!t||!t.closest){return;}var modal=t.closest('.elementor-popup-modal');if(!modal){return;}var closeBtn=t.closest('.dialog-close-button,.elementor-popup-modal .dialog-lightbox-close-button,.eicon-close,[aria-label="Close"],[data-elementor-action="close"]');var overlay=t.classList&&(t.classList.contains('dialog-widget-overlay')||t.classList.contains('dialog-lightbox-close-button'));if(!closeBtn&&!overlay){return;}var pid=popupIdFromModal(modal);if(pid){forceClosePopup(pid);}},true);document.addEventListener('keydown',function(e){if(!e||e.key!=='Escape'){return;}var modals=document.querySelectorAll('.elementor-popup-modal');for(var i=0;i<modals.length;i++){if(modals[i].offsetParent===null){continue;}var pid=popupIdFromModal(modals[i]);if(pid){forceClosePopup(pid);break;}}},true);}
...
function isPopupVisible(pid){if(isPopupUserClosed(pid)){return false;}if(isPopupDocOpen(pid)){return true;}var nodes=document.querySelectorAll('#elementor-popup-modal-'+pid+',.elementor-popup-modal[data-elementor-id="'+pid+'"]');for(var i=0;i<nodes.length;i++){var st=window.getComputedStyle?window.getComputedStyle(nodes[i]):null;if(st&&st.display==='none'){continue;}if(st&&st.visibility==='hidden'){continue;}if(nodes[i].offsetParent!==null){return true;}}return false;}
function forceShowModalDom(pid){if(isPopupUserClosed(pid)||isSuppressingReopen(pid)){return false;}var modal=findPopupModal(pid);if(!modal){return false;}modal.style.setProperty('display','flex','important');modal.style.setProperty('visibility','visible','important');modal.style.setProperty('opacity','1','important');modal.removeAttribute('aria-hidden');try{document.body.classList.add('elementor-popup-modal-open');}catch(e){}return isPopupVisible(pid);}
function openAllowedPopup(pid){pid=norm(pid);if(!pid||shouldBlockOpen(pid)){return false;}if(isPopupUserClosed(pid)||isSuppressingReopen(pid)){dbg('skip open user-closed/suppress',pid);return false;}clearEgpReopenBlock(pid);clearPopupDisableStorage(pid);patchRuntime();var doc=ensurePopupDocument(pid);var mod=window.elementorProFrontend&&elementorProFrontend.modules&&elementorProFrontend.modules.popup;dbg('open attempt',pid,'doc=',!!doc,'ready=',isPopupDocReady(doc),'egp_closed=',isPopupUserClosed(pid));if(doc&&isPopupDocReady(doc)&&typeof doc.showModal==='function'){try{doc.showModal();}catch(e){dbg('showModal error',pid,e&&e.message?e.message:e);}}else if(mod&&typeof mod.showPopup==='function'){mod.showPopup({id:pid});}if(isPopupUserClosed(pid)||isSuppressingReopen(pid)){hideForcedModalDom(pid);return false;}if(!isPopupVisible(pid)&&doc){forceOpenViaModal(doc,pid);}if(!isPopupVisible(pid)){forceShowModalDom(pid);}return isPopupVisible(pid);}
function tryPageLoadFallbackFor(pid,m){if(fallbackDone[String(pid)]||fallbackStarted[String(pid)]){return;}fallbackStarted[String(pid)]=1;var delay=parseInt(m.page_load_delay_ms,10)||0;var attempt=0;var maxAttempts=20;var tick=function(){if(fallbackDone[String(pid)]){return;}if(isPopupUserClosed(pid)){fallbackDone[String(pid)]=1;dbg('fallback stop user-closed',pid);return;}patchRuntime();if(isPopupVisible(pid)){fallbackDone[String(pid)]=1;dbg('fallback open ok',pid);return;}attempt++;var fdoc=getPopupDoc(pid);dbg('fallback attempt',pid,attempt,'modal=',!!findPopupModal(pid),'doc=',!!fdoc,'ready=',isPopupDocReady(fdoc),'egp_closed=',isPopupUserClosed(pid));if(isPopupUserClosed(pid)){fallbackDone[String(pid)]=1;return;}openAllowedPopup(pid);if(isPopupVisible(pid)){fallbackDone[String(pid)]=1;dbg('fallback open ok',pid);return;}if(attempt>=maxAttempts){fallbackDone[String(pid)]=1;dbg('fallback gave up',pid,'modal=',!!findPopupModal(pid),'doc=',!!getPopupDoc(pid),'markup=',!!findPopupMarkup(pid));return;}setTimeout(tick,300);};setTimeout(tick,Math.max(delay,800));}
