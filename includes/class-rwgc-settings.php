...
class RWGC_Settings {
	...
	public static function sanitize_settings( $input ) {
		...
		$out['maxmind_account_id']  = isset( $settings['maxmind_account_id'] ) ? sanitize_text_field( $settings['maxmind_account_id'] ) : sanitize_text_field( $preserved['maxmind_account_id'] );
		$out['maxmind_license_key'] = isset( $settings['maxmind_license_key'] ) ? sanitize_text_field( $settings['maxmind_license_key'] ) : sanitize_text_field( $preserved['maxmind_license_key'] );
		$out['auto_update_db']      = array_key_exists( 'auto_update_db', $settings ) ? ( ! empty( $settings['auto_update_db'] ) ? 1 : 0 ) : ( ! empty( $preserved['auto_update_db'] ) ? 1 : 0 );
		...
		$out['db_last_updated']     = isset( $settings['db_last_updated'] ) ? sanitize_text_field( $settings['db_last_updated'] ) : sanitize_text_field( $preserved['db_last_updated'] );
		$out['db_file_path']        = isset( $settings['db_file_path'] ) ? sanitize_text_field( $settings['db_file_path'] ) : sanitize_text_field( $preserved['db_file_path'] );
		$out['db_last_error']       = isset( $settings['db_last_error'] ) ? sanitize_text_field( $settings['db_last_error'] ) : sanitize_text_field( $preserved['db_last_error'] );
		$out['migration_completed'] = array_key_exists( 'migration_completed', $settings ) ? ( ! empty( $settings['migration_completed'] ) ? 1 : 0 ) : ( ! empty( $preserved['migration_completed'] ) ? 1 : 0 );
